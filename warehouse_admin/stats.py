...
from warehouse.core.constants import CHECKPOINT_TABLE, JobStatus, JobType
...
def _instrument_counts(handles: WarehouseHandles) -> tuple[Optional[int], Optional[int], bool]:
    ...
    try:
        active = registry.list_active_instruments()
    except InstrumentMasterUnavailableError:
        return None, None, False

    # get_instrument()/list_active_instruments() are the only public
    # registry methods — total count (active + inactive) isn't directly
    # exposed, so we query it the same read-only way the registry itself
    # would, reusing its own connection helper rather than adding a new
    # public method to a NGWH-001 file that's frozen.
    try:
        import sqlite3
