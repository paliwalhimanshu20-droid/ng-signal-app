...
# Segments that are entirely non-tradable reference feeds regardless of
# instrument_type. Added after a real production run's failure detail
# showed 13 rows (26 failures — lot_size + tick_size) all under
# 'GLOBAL_INDEX'/'GLOBAL_INDICATOR' with instrument_type='UNKNOWN': keys
# like GLOBAL_INDEX|^HSI, GLOBAL_INDEX|^DJI, GLOBAL_INDEX|SGX NIFTY,
# GLOBAL_INDICATOR|USDINR, GLOBAL_INDICATOR|BZUSD. Every one of them is a
# globally recognized macro/international market indicator (Hang Seng,
# Dow, FTSE, Nikkei, DAX, CAC40, S&P 500, SGX Nifty, USDINR, Brent/WTI
# crude) — Upstox's well-known "Global Markets" reference ticker set that
# Indian brokers ship for pre-market context. These never came from any
# NGSP seeding code (confirmed: no reference to these segment names
# anywhere in this repo) — they are genuinely part of Upstox's own
# complete.json.gz feed. Never tradable, so lot_size=tick_size=0 is
# correct by design, not corruption. instrument_type is 'UNKNOWN' for
# these specifically because Upstox has no real instrument_type for a
# non-tradable reference feed — which is exactly why segment, not
# instrument_type, is the reliable signal here.
NON_TRADABLE_SEGMENTS = {"GLOBAL_INDEX", "GLOBAL_INDICATOR"}

# If invalid lot_size/tick_size shows up on a row NOT covered by either
# exception set above (i.e. something that should be tradable — EQ, FUT,
# CE, PE, COM, ...), that is treated as a genuine integrity concern
# (FAIL), not a benign quirk. Unlike the duplicate-contract check, no
# percentage tolerance applies here — even one such row means the source
# record itself is broken.
...
def _lot_or_tick_rule(conn, column: str, rule_name: str) -> list[ValidationIssue]:
    ...
    for r in cur.fetchall():
        ...
        elif seg in NON_TRADABLE_SEGMENTS:
            severity = Severity.WARNING
            reason = (
                f"segment='{seg}' is one of Upstox's non-tradable Global Markets reference feeds "
                f"(instrument_type is 'UNKNOWN' precisely because these aren't real tradable "
                f"instruments) — {column}=0 is correct by design, not corruption."
            )
        else:
            severity = Severity.FAIL
            reason = (
                f"instrument_type='{itype or 'UNKNOWN'}', segment='{seg or 'UNKNOWN'}' is expected "
                f"to be tradable and must have a positive {column}. No known benign explanation — "
                f"treated as real data corruption."
            )
        issues.append(ValidationIssue(
            rule=rule_name,
            severity=severity,
